import pytest
import numpy
from pynds.relation import minimizing_dominance_relationship
...
def test_minimizing_dominance_relationship_1d():
    ...
    assert minimizing_dominance_relationship(x, y) == -1

    # x is nondominated by y
    x = numpy.array([0.0], dtype = float)
    y = numpy.array([0.0], dtype = float)
    assert minimizing_dominance_relationship(x, y) == 0
